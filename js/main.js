...
(function () {
  ...
  // If a newly deployed image is temporarily unavailable, show a known local fallback
  // instead of leaving a broken media frame on the page.
  [].slice.call(document.querySelectorAll('img[data-fallback-src]')).forEach(function (img) {
    var fallbackSrc = img.getAttribute('data-fallback-src');
    var showFallback = function () {
      if (!fallbackSrc || img.getAttribute('src') === fallbackSrc) return;
      img.setAttribute('src', fallbackSrc);
      img.classList.add('media-fallback-active');
    };
